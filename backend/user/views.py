...
def login(request):
    if authenticate(username=request.data['username'], password=request.data['password']):
        ...
        if factory_member_ser['status'].value == 'ENABLED':
            ...
            return Response('User disabled.', status=status.HTTP_403_FORBIDDEN)
    else:
...
def users_list_all(request):
    ...
    resp = {}
    factories = admin_ser['factory'].value
    if 0 in factories:
        user_list = facmodels.FactoryMember.objects.all()
        user_list_ser = facserializers.FactoryMemberAllSerializers(user_list, many=True)
        resp = user_list_ser.data
    else:
        for factory in factories:
            factory_name = facmodels.Factory.objects.get(id=factory).name
            print(factory_name)
            resp = []
            if "sections" in request.data:
                for sections in request.data['sections']:
                    user_list = facmodels.FactoryMember.objects.filter(factory__id__icontains=factory,
                                                                       ...
                                                                       status="ENABLED")
                    user_list_ser = facserializers.FactoryMemberAllSerializers(user_list, many=True)
                    resp.append({sections: user_list_ser.data})
